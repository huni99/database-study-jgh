-- Active: 1749600516352@@127.0.0.1@3306@quiz9
-- 1. 서브쿼리 확인 문제
...
-- 정답:
		...
		GROUP BY e.name , e.id; -- 중복이름이 있을수도 있으니 id까지 같이 묶어서 그룹화

		-- LEFT JOIN을 사용하면 프로젝트 참여하지  않은 인원도 표시할수있음
		SELECT name,  (
...
-- 정답: 
		...
		SELECT departments.name ,COUNT(e.id)
		FROM employees e
		JOIN departments ON departments.id= e.department_id
		GROUP BY departments.name
 
	SELECT b.name ,COUNT(b.i)
	FROM (
		SELECT departments.id i ,name  
		FROM departments
		) b 
		JOIN employees e ON e.department_id= b.i 
		GROUP BY b.name
...
SELECT name m,salary
FROM employees e
JOIN (SELECT MAX(amount) m
			FROM salary_records	 ) se ON e.salary= se.m;
...
-- 정답:
	SELECT k
	FROM (SELECT COUNT(ep.employee_id) t, p.name k
		FROM employee_projects ep 
