-- Active: 1749600516352@@127.0.0.1@3306@bank
/*
4. 데이터 집계하기
4.1 집계 함수(Aggregate Function)란
*/
-- 무엇? 통게적 계산을 간편하게 수행하도록 돕는 기능
...
-- COUTN(): 레코드(튜플)의 개수를 반환(예: 학생 테이블에 총 학생이 몇 명 있는지 셀 때)
-- SUM(): 합계 반환( 예: 장바구니에 담긴 모든 아이템들의 가격 총합을 계산 할 때)
-- AVG(): 평균 반환(예: 학생들의 수학 점수 평균을 구할 때)

--실습 데이터 준비
--3장에서 연습한 맵도날드 db를 활용
USE mapdonalds;

--가장 비싼 버거와 가장 싼버거의 가격은?
--최대값, 최소값
--MAX(), MIN()
...
--집계 함수를 다른 일반 컬럼들과 함계 사용할 때는 GROUP BY가 필요
