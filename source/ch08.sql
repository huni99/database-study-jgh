...
--
SELECT  city ,SUM(amount),YEAR(sale_date) -- 3) 출력
FROM sales -- 1) sales 테이블을
GROUP BY city , YEAR(sale_date); -- 2) 도시와 연도로 그룹화한 

-- 3) select 절에 올 수 있는 컬럼이 제한적이다
-- 사용 가능한 컬럼: 
-- 그룹화 컬럼: group by절에서 지정한 컬럼
...
SELECT 그룹화_컬럼, 집계_함수(일반_컬럼)
FROM 테이블명
WHERE 일반_필터링_조건 -- 그룹화 하기 전 개별 행(row)에 대해 필터링
GROUP BY 그룹화_컬럼
HAVING 그룹_필터링 조건; -- 그룹핑된 결과에 대해 필터링하는 그룹 단위 필터링

--결제 유형별 평균 결제 금액이 40000원 이상인 데이터는?

SELECT AVG(amount),ptype
FROM payments
...
-- ORDER BY 절을 사용하여 수행
-- SLECT로 조회된 데이터를 기준으로 정렬하는 작업(SLECT가 먼저 수행됨)
...
SELECT ptype '결제 유형' , amount  '결제 금액'
FROM payments  
...
SELECT ptype '결제 유형' , amount  '결제 금액'
FROM payments  
ORDER BY ptype ASC , amount DESC; -- ptype의 오름차순 정렬후 , amount의 내림차순 정렬(순서 중요)
-- ORDER BY amount DESC , ptype ASC; -- 이러면 그냥 amount의 내림차순만 된다
...
SELECT 컬럼1, 컬럼2
FROM 테이블명
LIMIT N OFFEST M;-- N: 가져올 레코드의 개수, M: 건너뛸 레코드의 개수
-- 또는  LIMIT M ,N;
...
VALUES
  ...
  SELECT product_type,COUNT(*) '상품의 개수', MAX(price)  '최고 가격', MIN(price) '최저 가격'
  FROM products
  GROUP BY product_type;

  --사용자별 주문 총액을 구하고, 그 총액이 30000원 이상인 주문자는?
  SELECT *
FROM users u
...
GROUP BY name; -- 상품별 그룹화
  ...
  SELECT name '상품명' , SUM(count) '판매수량' --6
  FROM order_details -- 1
  JOIN orders ON orders.id = order_details.order_id -- 2
  JOIN products ON product_id=products.id -- 3
