...
-- 단백질량이 25g 보다 많지 않은 버거
  ...
  --3)산술 연산자
  -- 사칙연산 등을 위한 수학적 연산 기호 
  -- WHERE절과 SELECT 절에서 사용가능
  SELECT 100+20;
  SELECT 100-20;
  SELECT 100*20;
  SELECT 100/20;
...
-- students 데이터 삽입
INSERT INTO students(id, name, math, english, programing)
VALUES
	(1, 'Sparkles', 98, 96, 93),
	(2, 'Soldier', 82, 66, 98),
...
ALTER TABLE students RENAME COLUMN name TO nickname;
...
-- 1. 모든 과목 성적이 90점 이상인 학생은?
  SELECT *
  FROM students
  WHERE math>=90 AND english >=90 AND programing >=90;

-- 2. 75점 미만이 하나라도 있는 학생은?
  SELECT *
  FROM students
  WHERE math<=75 OR english <=75 OR programing <=75;

-- 3. 모든 학생의 총점은?

  SELECT * , math+english+programing AS '총점'
  FROM students ;

-- 4. 모든 학생의 평균은?
  SELECT * , (math+english+programing)/3 AS '평균'
  FROM students ;

-- 5. 총점이 270 이상인 학생의 닉네임, 총점, 평균은?

  SELECT nickname AS '닉네임',math+english+programing AS '총점' , (math+english+programing)/3 AS '평균'
  FROM students 
  WHERE math+english+programing>=270;
