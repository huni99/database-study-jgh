-- Active: 1749600516352@@127.0.0.1@3306@stargram
/*
6. 관계 만들기
6.1 관계란
*/
...
-- 주의: 제약 효과는 같을 수 있지만, PK는 구조적, 의미적으로도 특별한 역할을 가져서 완벽하게 똑같지는 않다
...
CREATE TABLE users (
  ...
  age TINYINT CHECK (age>=18), -- 나이 (18세 이상만 허용)
...
CREATE TABLE products (
  ...
  dc_rate INTEGER CHECK (dr_rate BETWEEN 0 AND 50),      -- 할인율(0~50% 제한)
...
6.2 다양한 관계 만들기
*/
-- 외래키와 기본키를 연결하여 일애일/일대다/다대다 관계를 만들어 보자
...
CREATE TABLE capitals(
  ...
  -- 타입은 국가 테이블과 동일하게 맞춤
  -- 고유한 값만 허용 (한 나라에 수도가 2개가 있으면 안됨) = > UNIQUE : 일대일 관계를 만들기 위한 중요한 제약

  PRIMARY KEY(id),
  Foreign Key (country_id) REFERENCES countries(id) -- 외래키 지정 :country_id 
  -- country_id 외래키는 countries 테이블의 id 컬럼을 가리킨다는 의미(참조)
);
-- countries 데이터 삽입
...
-- 하나의 데이터에 여러 데이터가 포하되거나 소유 되는 경우
...
CREATE TABLE players(
  id INTEGER,
  name VARCHAR(255),
  team_id INTEGER, -- 소속팀 아이디, 일대다 관계에서는 UNIQUE 제약조건 x
  PRIMARY KEY(id),
...
CREATE TABLE appointments(
  ...
  patient_id INTEGER ,
  date DATE, -- 진료일자
  Foreign Key (doctor_id) REFERENCES patients(id),
