-- Active: 1749600516352@@127.0.0.1@3306@store
/*
5. 다양한 자료형 활용하기
5.1 자료형이란
*/
...
-- FLOAT과 DOUBLE 타입은 0.1을 정확시 저장하지 않고 근사값으로 저장(왜? 0.1은 2진수로 무한소수)
-- FLOAT: 약 7자리 정확도
...
CREATE TABLE student_records(
  ...
  average_score FLOAT, -- 평군 점수(부동 소수점 방식의 실수)
...
CREATE TABLE addresses(
  ...
  -- 최대 100글자 저장 가능 하지만, 사용 메모리는 입력된 문자만큼 사용(예 'abc')
  );
...
CREATE TABLE acticles(
  ...
  additional_info LONGTEXT -- 추가정보 (최대 4GB)
);
...
-- BLOB 자료형의 사용 예
CREATE TABLE files(
  file_name VARCHAR(200), -- 파일명(가변길이 문자 : 최대 200자)
  smal_thumbnail TINYBLOB, -- 작은 이미지 파일( 최대 255Byte)
...
-- 데이터베이스에 직접 저장하지 않고, 클라우드 스토리지나 파일 서버 등에 저장한뒤,
-- 그경로나 URL만 데이터베이스에 저장하는 방식이 사실상 표준
...
CREATE TABLE user_profiles (
  ...
  phone CHAR(13),  -- 전화번호(고정 길이 문자: 13자)
  info TEXT,  -- 자기소개(긴 문자열: 최대 64KB)
  profile_img MEDIUMBLOB, -- 프로필 사진(파일: 최대 16MB)
...
INSERT INTO 
	user_profiles (id, email, phone, info, profile_img, gender)
VALUES
	(1, 'geoblo@naver.com', '012-3456-7890', '안녕하십니까!', NULL, '남'),
	(2, 'hongsoon@example.com', '098-7654-3210', '반갑습니다요!', NULL, '여');
...
INSERT INTO orders (id, name, price, quantity, created_at)
VALUES (23, 'ㄴ', 9520.25, 1, '2024-12-11 12:34:56');
-- 데이터 조회
SELECT * FROM orders;
...
SELECT *
FROM orders
-- WHERE   (name BETWEEN  'ㄱ ' AND 'ㄴ') AND not name = 'ㄴ';
WHERE name < 'ㄴ';
