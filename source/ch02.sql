...
-- );
	CREATE TABLE burgers(
		id INTEGER, -- 아이디(정수: -1,0,1...) INT도 허용 
        name VARCHAR(50), -- 이름(최대 50글자의 문자: '빅맥', '상하이 치즈버거',...)
...
-- 컬럼 생략 가능? 가능 하지만 테이블 정의와 정확히 일치해야 함
-- 항상 컬럼을 명시하는 방식을 권장

-- 데이터 조회하기
-- SELECT 컬럼명1, 컬럼명2, ... -- 컬럼명 대신 *을 쓰면 모든 컬럼 조회
-- FROM 테이블명
-- WHERE 조건, 검색조건(생략하면 전체 조회) 
...
-- '슈비두밥 버거' 단종됐다면, 이를 위한 데이터 삭제 쿼리는?
DELETE FROM burgers
